...
std::string getKey(const std::string &key)
{
  ...
  const std::string efusekeyPathStr = "/sys/firmware/devicetree/base/efusekey";
  ...
  for (const auto& dirEntry : std::filesystem::directory_iterator(path, ec)) {
    ...
    const auto startsWithKeyUnderscore = filenameStr.rfind("key_", 0) == 0;
    //std::cout << "Iterating " << filenameStr << std::endl;
    if (dirEntry.is_directory() && startsWithKeyUnderscore) {
      const auto keynamePathStr = (efusekeyPathStr + "/" + filenameStr + "/keyname");
      const auto keyname = loadFileAsLine(keynamePathStr);
      //std::cout << keynamePathStr << " == '" << keyname << "'" << std::endl;
      if (keyname == key) {
        const uint32_t offset = castBytesInFile(efusekeyPathStr + "/" + filenameStr + "/offset");
        const uint32_t size = castBytesInFile(efusekeyPathStr + "/" + filenameStr + "/size");
